...
const Column = ({ defaultName, identifier }: Props) => {
    ...
    const addCard = (): void => {
        ...
        const newCard = { cardName, userName, comments: [{userName: 'me', commentsText: 'ypu'}, {userName: 'me', commentsText: 'ypu'}, {userName: 'me', commentsText: 'ypu'}], description: '', columnName: identifier };
    ...
    return(
        <ColumnContainer>
            ...
            {show && <Modal clickLayout={clickLayout} cardInModal={cardInModal} columnInModal={columnInModal} cardsList={cardsList} setCardsList={setCardsList}/>}
            <Section>
            ...
            <Section>
                {cardsList.map((item: ICard, key: number) => {
                    return <Card key={key} card={item} deleteCard={deleteCard} clickCardTitle={clickCardTitle} identifier={identifier} />
                })}
            </Section>
        </ColumnContainer>
    )
